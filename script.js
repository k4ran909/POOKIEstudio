...
function files(index) {
  var data = `
     ...
     ./asset/male0300.png
 `;
  // Build a clean array of non-empty, trimmed paths
  const list = data.split(/\r?\n/).map(s => s.trim()).filter(Boolean);
  return list[index];
}

// Build file list once and use its length for frameCount
...
const frameCount = fileList.length;
...
// If first image loads, render; otherwise attempt render safely
if (images[0]) images[0].onload = render;

function render() {
  const img = images[imageSeq.frame];
  if (!img || !img.complete || img.naturalWidth === 0) {
    // image not ready yet; try to draw a fallback or skip
    return;
  }
